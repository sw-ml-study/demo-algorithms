...
- `include` is forbidden in `web/*.mlpl`; Web UI Editor → Load → Run requires
  standalone files. It remains allowed under `src/`, `demos/`, and `tests/`.
- Keep decisions pure, runners thin, catalogs/docs current, and never modify
  `../sw-mlpl` or overwrite stable binaries.
