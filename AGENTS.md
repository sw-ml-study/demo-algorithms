...
- Exercise empty, singleton, boundary, duplicate, and invalid cases where
  applicable.
- Document dynamic-size behavior, representation invariants, logical
  complexity, current copy complexity, and explicit-loop count.
- Prefer array operations, pure functions, recursion, and delegation. Explain
  every remaining explicit loop and the future combinator that could remove it.
- Do not claim structural sharing when the current runtime copies values.
- Use numeric IDs/handles for logical cycles. Application-created leaks or
  exhaustion are application bugs.
- Treat strings as static messages/labels only with the current language.

## Scope and references

- Capability analysis: `docs/analysis.md`
- Demo and feature plan: `docs/plan.md`
