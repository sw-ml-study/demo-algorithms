...
continuation, and deterministic exit status all work now. All 56 registered
tests and all 59 demos share production definitions under `src/`. See
[mlplunit-adoption.md](mlplunit-adoption.md) and
[mlplunit-migration.md](mlplunit-migration.md).
The current native event transport reports 80 individual tests/cases from the
56 files in human and TAP modes. A nonblocking refinement backlog remains:
