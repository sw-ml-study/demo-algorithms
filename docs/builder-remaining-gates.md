...
sw-MLPL 0.20.0 binary. Four GoF patterns do not yet have dedicated executable
...
1. Prototype a fixed nested Chain only if delegation and short-circuit behavior
   can be shown without a central handler opcode switch.
2. Keep Singleton gated until modules exist; an `include` file is source reuse,
   not evaluate-once identity.
3. Treat dynamic Chain, Observer, and Mediator as primary acceptance consumers
