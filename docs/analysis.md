...
| 8 | Tighten text/byte policies, then versioned native serialization | 4 | 4 | 2 | MLPB v1 now deterministically preserves every data kind and exact array shape with bounded decode; tagged JSON, TOML-subset interoperability, numeric-byte persistence, and atomic replacement run; reference/cycle policy, streaming, TOML tagged mode, and stronger integrity remain; see `serialization-acceptance.md` |
