...
0.20.0 build `8f88012e`, plus mlplunit `a06191f`,
...
| Sparse matrix composition | deterministic CSR output | Runnable addition and rectangular multiplication with dense-oracle checks; implementation scans coordinate domains rather than claiming optimized sparse accumulation |
...
| 8 | Tighten text/byte policies, then versioned native serialization | 4 | 4 | 2 | MLPB v2 deterministically preserves every data kind and exact array shape with bounded decode and CRC32 while retaining v1 reads; tagged JSON, TOML-subset interoperability, numeric-byte persistence, and atomic replacement run; reference/cycle policy, streaming, TOML tagged mode, and authenticated integrity remain; see `serialization-acceptance.md` |
